...
window.onerror = function (msg, url, lineNo, columnNo, error) {
    ...
    // Only alert on mobile to avoid annoying desktop users
    if (/iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) {
        alert("Fatal Error tracked: " + msg + " at line " + lineNo);
    }
    return false;
};
...
// Navigation logic is handled in the first showScreen definition above
...
const translations = {
    ru: {
        ...
        how_to_install_title: "📱 Как установить?",
        view_instructions: "Посмотреть инструкцию",
    ...
    en: {
        ...
        notifications_desc: "Get alerts about visa status",
        how_to_install_title: "📱 How to install?",
        view_instructions: "View Instructions",
        install_guide_title: "Installation Guide",
        guide_android: "Tap the three dots in the browser corner and select 'Install app' or 'Add to Home screen'.",
        guide_ios: "Tap the 'Share' button (square with arrow) and select 'Add to Home Screen'.",
        guide_desktop: "Click the monitor icon with an arrow in the browser's address bar.",
...
function scanDocument() {
    ...
    if (uploadArea) uploadArea.classList.add('scanning');

    // Dynamic loader messages
    const messages = [
        "AI анализирует структуру документа...",
        "Извлечение текста и проверка подлинности...",
        "Сверка с требованиями выбранной страны...",
        "Расчет вероятности одобрения..."
    ];
    let msgIndex = 0;
    const msgInterval = setInterval(() => {
        loaderText.textContent = messages[msgIndex % messages.length];
        msgIndex++;
    }, 1500);

    // Simulate AI processing (increased for more "premium" feel)
    setTimeout(() => {
        clearInterval(msgInterval);
        if (uploadArea) uploadArea.classList.remove('scanning');
        performAIScan();
    }, 6000);
}

// Perform AI scan with Gemini AI (always enabled)
async function performAIScan() {
    ...
    // Step 1: Document Validation and OCR
    const ocrPrompt = `Проанализируй это изображение. Это должен быть визовый документ (паспорт, справка, бронь и т.д.).
    
    1. Если на изображении НЕТ документа (пустой лист, другой предмет, размытое пятно), ответь СТРОГО словом "INVALID: [подробная причина на русском]".
    2. Если документ есть, определи его тип и извлеки весь видимый текст.
    
    Верни только ИЗВЛЕЧЕННЫЙ ТЕКСТ или сообщение "INVALID: [причина]".`;

    const ocrResult = await callGemini(ocrPrompt, imageData);

    if (!ocrResult || ocrResult.includes('INVALID:')) {
        isValidDocument = false;
        invalidReason = ocrResult ? ocrResult.replace('INVALID:', '').trim() : 'Документ не распознан';
        console.warn('⚠️ Валидация не прошла:', invalidReason);
    } else {
    ...
    if (isValidDocument) {
        ...
        const analysisPrompt = `Ты эксперт по визовым правилам. Проанализируй этот документ для визы типа "${scannerState.selectedVisaType}" в страну "${countryName}".

ИЗВЛЕЧЕННЫЙ ТЕКСТ ИЗ ДОКУМЕНТА:
${extractedText || 'Текст не распознан'}

ТРЕБОВАНИЯ ДЛЯ ЭТОЙ ВИЗЫ:
${requirements.join('\n')}

Проанализируй документ на наличие ошибок, соответствие требованиям и полноту данных.
Верни ответ СТРОГО в формате JSON (без markdown блоков):
{
  "hasIssues": boolean,
  "issues": ["проблема 1", "проблема 2"],
  "missingDocs": ["недостающий документ 1"],
  "approvalProbability": число от 0 до 100,
  "reasoning": "краткое экспертное объяснение оценки на русском языке"
}

Критерии оценки одобрения:
- 90-100%: Идеальный документ, все данные на месте.
- 70-89%: Есть мелкие недочеты, не критично.
- 40-69%: Серьезные ошибки или не хватает важных данных.
- 0-39%: Критические нарушения или документ не подходит.`;

        const aiResultRaw = await callGemini(analysisPrompt, imageData);

        if (aiResultRaw) {
            try {
                const cleanJson = aiResultRaw.replace(/```json|```/g, '').trim();
                const aiResult = JSON.parse(cleanJson);
                hasIssues = aiResult.hasIssues || false;
                issues = aiResult.issues || [];
                missingDocs = aiResult.missingDocs || [];
                approvalProbability = aiResult.approvalProbability || 0;

                // Extra check: if AI says 0 but didn't say it's invalid
                if (approvalProbability < 10 && issues.length === 0) {
                    isValidDocument = false;
                    invalidReason = aiResult.reasoning || 'Это не похоже на визовый документ';
                }

                console.log('✅ AI Анализ успешен:', aiResult);
            } catch (e) {
                console.error("❌ Ошибка парсинга AI результата:", e);
                approvalProbability = 50;
                hasIssues = true;
                issues = ["Не удалось разобрать ответ от AI"];
            }
        }
    }

    // Display results (passing invalid state)
    displayResults(hasIssues, issues, requirements, countryName, missingDocs, approvalProbability, isValidDocument, invalidReason);

    // Save to history only if it was a valid attempt at a document
    if (isValidDocument) {
        saveDocumentToHistory({
            date: new Date().toLocaleDateString(),
            country: countryName,
            type: scannerState.selectedVisaType,
            fileName: scannerState.uploadedFileName,
            status: hasIssues ? 'error' : (missingDocs.length > 0 ? 'warning' : 'success'),
...
function saveDocumentToHistory(doc) {
    // Get the document image data
    const imageData = document.getElementById('preview-image').src;

    // Add image data to document
    doc.imageData = imageData;

    let history = JSON.parse(localStorage.getItem('visa_doc_history') || '[]');
    history.unshift(doc); // Add to beginning
    localStorage.setItem('visa_doc_history', JSON.stringify(history.slice(0, 20))); // Keep last 20
    updateDocumentList();
}
...
function updateDocumentList() {
    ...
    // Clear existing (except maybe a few defaults if we want to keep them, but let's replace)
    docList.innerHTML = '';

    if (history.length === 0) {
        docList.innerHTML = '<p style="text-align: center; padding: 20px; opacity: 0.5;">Нет сохраненных документов</p>';
        return;
    }
    ...
    history.forEach((doc, index) => {
        ...
        item.innerHTML = `
            <div class="document-content" onclick="viewDocument(${index})" style="cursor: pointer; flex: 1;">
                <h4>${doc.fileName || 'Без названия'}</h4>
                <p style="font-size: 12px; opacity: 0.7;">${doc.country} (${doc.type}) • ${doc.date}</p>
            </div>
            <button class="document-action" onclick="viewDocument(${index})" aria-label="View" title="Просмотреть документ">
                <span class="action-icon">👁️</span>
            </button>
            <button class="document-action delete-btn" onclick="deleteDocument(${index})" aria-label="Delete" title="Удалить документ">
                <span class="action-icon">🗑️</span>
            </button>
            <div class="document-status" title="Статус проверки">${statusMap[doc.status]}</div>
        `;
...
function viewDocument(index) {
    ...
    // Set metadata
    viewerTitle.textContent = doc.fileName || 'Без названия';
    viewerMeta.innerHTML = `
        <p><strong>Страна:</strong> ${doc.country}</p>
        <p><strong>Тип визы:</strong> ${doc.type}</p>
        <p><strong>Дата:</strong> ${doc.date}</p>
        ${doc.approvalProbability ? `<p><strong>Вероятность одобрения:</strong> ${doc.approvalProbability}%</p>` : ''}
    `;

    // Set status
    const statusMap = {
        'success': { icon: '🟢', text: 'Документ прошел проверку' },
        'warning': { icon: '🟡', text: 'Требуется внимание' },
        'error': { icon: '🔴', text: 'Обнаружены ошибки' }
    };

    const status = statusMap[doc.status] || statusMap.warning;
    viewerStatus.innerHTML = `${status.icon} ${status.text}`;
...
function deleteDocument(index) {
    ...
    const confirmMessages = {
        ...
        zh: '您确定要删除此文档吗？',
        ar: 'هل أنت متأكد أنك تريد حذف هذا المستند؟',
        hi: 'क्या आप वाकई इस दस्तावेज़ को हटाना चाहते हैं?'
    };

    const confirmMessage = confirmMessages[lang] || confirmMessages.ru;
    ...
    localStorage.setItem('visa_doc_history', JSON.stringify(history));
...
// Display scan results with approval probability
function displayResults(hasIssues, issues, requirements, countryName, missingDocs, approvalProbability = 50, isValidDocument = true, invalidReason = '') {
    // Set status
    ...
    if (!isValidDocument) {
        ...
        statusText.textContent = lang === 'ru' ? 'Документ не распознан' : 'Document not recognized';

        // Clear and show error message
        const resultStatus = document.getElementById('result-status');
        const existingProb = resultStatus.querySelector('.approval-probability');
        if (existingProb) existingProb.remove();

        const errorHTML = `
            <div class="approval-probability" style="margin-top: 20px; padding: 20px; background: rgba(244, 67, 54, 0.1); border-radius: 15px; text-align: center;">
                <h3 style="margin: 0 0 10px 0; font-size: 16px; color: #f44336;">
                    ${lang === 'ru' ? '⚠️ Ошибка сканирования' : '⚠️ Scan Error'}
                </h3>
                <p style="margin: 0; font-size: 14px; color: #666;">
                    ${invalidReason || (lang === 'ru' ? 'Пожалуйста, убедитесь, что на фото виден весь документ и текст читаем.' : 'Please ensure the entire document is visible and text is readable.')}
                </p>
                <p style="margin-top: 10px; font-weight: bold; color: #f44336;">
                    ${lang === 'ru' ? 'Проценты не начислены' : 'No percentage calculated'}
                </p>
            </div>
        `;
        resultStatus.insertAdjacentHTML('beforeend', errorHTML);

        document.getElementById('issues-section').style.display = 'none';
        document.getElementById('missing-docs-section').style.display = 'none';
    ...
    const resultStatus = document.getElementById('result-status');
    let probabilityHTML = `
        <div class="approval-probability" style="margin-top: 20px; padding: 20px; background: linear-gradient(135deg, rgba(76, 175, 80, 0.1), rgba(33, 150, 243, 0.1)); border-radius: 15px; text-align: center;">
            <h3 style="margin: 0 0 15px 0; font-size: 16px; color: #555;">
                ${lang === 'ru' ? '🎯 Вероятность одобрения визы' : '🎯 Visa Approval Probability'}
            </h3>
            <div style="position: relative; width: 100%; height: 120px; margin: 15px 0;">
                <svg viewBox="0 0 200 120" style="width: 100%; height: 100%;">
                    ...
                    <path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" 
                          stroke="${approvalProbability >= 75 ? '#4CAF50' : approvalProbability >= 50 ? '#FF9800' : '#f44336'}" 
                          stroke-width="12" 
                          stroke-linecap="round"
                          stroke-dasharray="${approvalProbability * 2.51}, 251"
                          style="transition: stroke-dasharray 1s ease-in-out;"/>
                    <!-- Percentage text -->
                    <text x="100" y="85" text-anchor="middle" font-size="36" font-weight="bold" 
                          fill="${approvalProbability >= 75 ? '#4CAF50' : approvalProbability >= 50 ? '#FF9800' : '#f44336'}">
                        ${approvalProbability}%
                    </text>
                </svg>
            </div>
            <p style="margin: 10px 0 0 0; font-size: 14px; color: #666;">
                ${approvalProbability >= 80 ? (lang === 'ru' ? '✅ Отличные шансы на одобрение!' : '✅ Excellent approval chances!') :
            approvalProbability >= 60 ? (lang === 'ru' ? '👍 Хорошие шансы на одобрение' : '👍 Good approval chances') :
                approvalProbability >= 40 ? (lang === 'ru' ? '⚠️ Требуется доработка документов' : '⚠️ Documents need improvement') :
                    (lang === 'ru' ? '❌ Высокий риск отказа' : '❌ High rejection risk')}
            </p>
        </div>
        ${invalidReason ? `
        <div class="ai-reasoning" style="margin-top: 15px; padding: 15px; background: var(--primary-soft); border-radius: 12px; border-left: 4px solid var(--primary);">
            <p style="margin: 0; font-size: 14px; font-style: italic; color: var(--text-primary);">
                <strong>🤖 AI Эксперт:</strong> ${invalidReason}
            </p>
        </div>` : ''}
    `;
    ...
    const existingProb = resultStatus.querySelector('.approval-probability');
    const existingReasoning = resultStatus.querySelector('.ai-reasoning');
    if (existingProb) existingProb.remove();
    if (existingReasoning) existingReasoning.remove();
    resultStatus.insertAdjacentHTML('beforeend', probabilityHTML);
...
function showInstallInstructions() {
    ...
    setTimeout(toggleInstallInstructions, 300);
}
...
function showInstallPromotion() {
    ...
    const createBtn = () => {
        const btn = installBtn.cloneNode(true);
    ...
    if (settingsBody && !settingsBody.querySelector('.install-app-btn')) {
        settingsBody.appendChild(createBtn());
    }
    if (profileForm && !profileForm.querySelector('.install-app-btn')) {
        profileForm.appendChild(createBtn());
    }
}
...
async function callGemini(prompt, imageData = null) {
    ...
    if (!apiKey) {
        console.warn('⚠️ Gemini API ключ не найден. Пожалуйста, добавьте ключ в настройках.');
        return null;
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Always enable real AI - no toggle needed
    // Remove the old toggle logic since AI is always on
});
...
async function handleSendMessage() {
    ...
    // Fallback to simulation if AI fails or no API key
    if (!aiResponse) {
        aiResponse = processAIResponse(text);
        // Simulate thinking time for "virtual" AI
...
// ===================================
// Help Section Detail Logic
const helpContent = {
    ru: {
        help_item_1: {
            title: "Типы документов для визы",
            content: `
                <p>Для оформления визы обычно требуются следующие основные документы:</p>
                <ul>
                    <li><b>Загранпаспорт:</b> должен быть действителен минимум 6 месяцев после даты окончания поездки.</li>
                    <li><b>Проездные документы:</b> бронь авиабилетов или других видов транспорта.</li>
                    <li><b>Подтверждение проживания:</b> бронь отеля, договор аренды или приглашение.</li>
                    <li><b>Финансовые гарантии:</b> выписка из банка, справка с работы о доходах.</li>
                    <li><b>Страховка:</b> медицинский полис, покрывающий весь период поездки.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Консульство должно убедиться в вашей личности, целях поездки и наличии средств для возвращения в свою страну.</p>
            `
        },
        help_item_2: {
            title: "Правила оформления документов",
            content: `
                <p>Основные правила, которые помогут избежать отказа:</p>
                <ul>
                    <li>Все копии должны быть четкими и читаемыми.</li>
                    <li>Фотографии должны соответствовать стандартам (обычно 3.5x4.5 см или 5x5 см, белый фон).</li>
                    <li>Документы на национальном языке часто требуют перевода на английский или язык страны назначения.</li>
                    <li>Не используйте степлер; используйте канцелярские скрепки.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Соблюдение правил оформления ускоряет процесс обработки заявления и снижает риск технических ошибок.</p>
            `
        },
        help_item_3: {
            title: "Инструкция по заполнению",
            content: `
                <p>Как правильно заполнять анкеты и заявления:</p>
                <ul>
                    <li>Используйте только черную или синюю ручку (если заполняете вручную).</li>
                    <li>Пишите печатными латинскими буквами.</li>
                    <li>Заполняйте все поля. Если поле не относится к вам, пишите "N/A" или "NONE" (согласно инструкции конкретной страны).</li>
                    <li>Даты обычно указываются в формате ДД/ММ/ГГГГ.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Правильное заполнение исключает двусмысленность и помогает визовому офицеру быстрее понять вашу ситуацию.</p>
            `
        },
        help_item_4: {
            title: "Загрузка совместных документов",
            content: `
                <p>Если вы подаете документы вместе с семьей или группой:</p>
                <ul>
                    <li>Загружайте общие документы (бронь отеля, приглашение) для каждого участника.</li>
                    <li>Убедитесь, что в бронировании указаны фамилии всех заявителей.</li>
                    <li>Свидетельства о браке или рождении подтверждают родственную связь.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Это подтверждает целостность вашей группы и общие цели поездки.</p>
            `
        },
        help_item_5: {
            title: "Связь с поддержкой",
            content: `
                <p>Вы можете получить помощь несколькими способами:</p>
                <ul>
                    <li><b>ИИ-Помощник:</b> наш чат-бот доступен 24/7 для ответов на базовые вопросы.</li>
                    <li><b>Email:</b> напишите нам на support@visam.app для сложных случаев.</li>
                    <li><b>Телефон:</b> горячая линия доступна в рабочее время.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Мы поможем вам разобраться в сложных требованиях и подготовить идеальный пакет документов.</p>
            `
        },
        help_item_6: {
            title: "Регистрация и подача",
            content: `
                <p>Этапы процесса после подготовки документов:</p>
                <ul>
                    <li>Регистрация в системе визового центра или посольства.</li>
                    <li>Запись на подачу (биометрия и интервью).</li>
                    <li>Оплата консульского и сервисного сборов.</li>
                    <li>Отслеживание статуса через наше приложение или сайт ВЦ.</li>
                </ul>
                <h4>Зачем это нужно?</h4>
                <p>Понимание этапов помогает правильно спланировать время и избежать очередей.</p>
            `
        }
    },
    en: {
        help_item_1: {
            title: "Visa Document Types",
            content: `
                <p>The following main documents are usually required to apply for a visa:</p>
                <ul>
                    <li><b>Passport:</b> must be valid for at least 6 months after the trip end date.</li>
                    <li><b>Travel documents:</b> flight bookings or other transport.</li>
                    <li><b>Proof of accommodation:</b> hotel booking, rental agreement, or invitation.</li>
                    <li><b>Financial guarantees:</b> bank statement, certificate of income.</li>
                    <li><b>Insurance:</b> medical policy covering the entire period of the trip.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>The consulate must verify your identity, the purpose of your trip, and that you have the funds to return to your country.</p>
            `
        },
        help_item_2: {
            title: "Document Rules",
            content: `
                <p>Basic rules to help avoid refusal:</p>
                <ul>
                    <li>All copies must be clear and readable.</li>
                    <li>Photos must meet standards (usually 3.5x4.5 cm or 5x5 cm, white background).</li>
                    <li>Documents in national language often require translation into English or the destination language.</li>
                    <li>Do not use a stapler; use paper clips.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>Following the rules speeds up the application processing and reduces the risk of technical errors.</p>
            `
        },
        help_item_3: {
            title: "Filling Instructions",
            content: `
                <p>How to fill out forms and applications correctly:</p>
                <ul>
                    <li>Use only black or blue pen (if filling by hand).</li>
                    <li>Write in block capital letters.</li>
                    <li>Fill in all fields. If a field does not apply to you, write "N/A" or "NONE".</li>
                    <li>Dates are usually in DD/MM/YYYY format.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>Correct filling eliminates ambiguity and helps the visa officer understand your situation faster.</p>
            `
        },
        help_item_4: {
            title: "Joint Document Upload",
            content: `
                <p>If you are applying with family or a group:</p>
                <ul>
                    <li>Upload shared documents (hotel booking, invitation) for each participant.</li>
                    <li>Ensure the names of all applicants are listed in the booking.</li>
                    <li>Marriage or birth certificates confirm kinship.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>This confirms the integrity of your group and shared travel goals.</p>
            `
        },
        help_item_5: {
            ...
            content: `
                ...
                <ul>
                    <li><b>AI Assistant:</b> our chatbot is available 24/7 for basic questions.</li>
                    <li><b>Email:</b> write to support@visam.app for complex cases.</li>
                    <li><b>Phone:</b> hotline is available during business hours.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>We help you navigate complex requirements and prepare a perfect package.</p>
            `
        },
        help_item_6: {
            title: "Registration and Submission",
            content: `
                <p>Stages of the process after preparing documents:</p>
                <ul>
                    <li>Registration in the visa center or embassy system.</li>
                    <li>Appointment (biometrics and interview).</li>
                    <li>Payment of consular and service fees.</li>
                    <li>Status tracking via our app or VFS website.</li>
                </ul>
                <h4>Why is it needed?</h4>
                <p>Understanding the stages helps to plan correctly and avoid queues.</p>
            `
        }
    }
};

function openHelpDetail(itemId) {
    const lang = localStorage.getItem('language') || 'ru';
    const content = (helpContent[lang] && helpContent[lang][itemId]) ||
        (helpContent.ru[itemId]);

    if (content) {
        document.getElementById('help-detail-title').textContent = content.title;
        document.getElementById('help-detail-content').innerHTML = content.content;
        document.getElementById('help-detail-modal').classList.add('active');
    }
}

function closeHelpDetail() {
    document.getElementById('help-detail-modal').classList.remove('active');
}

// Service Worker registration is handled in the middle of the file.
